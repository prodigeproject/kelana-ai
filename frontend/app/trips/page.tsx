    ...
    <main className="min-h-screen bg-slate-50 px-6 py-10 text-slate-950 sm:px-10">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-10 flex flex-col gap-3 sm:flex-row">
          <input className="flex-1 rounded-xl border border-slate-300 bg-white px-4 py-3 outline-none focus:border-cyan-600" placeholder="Search trips..." value={search} onChange={(event) => setSearch(event.target.value)} />
          <select className="rounded-xl border border-slate-300 bg-white px-4 py-3" value={sort} onChange={(event) => setSort(event.target.value)}><option value="latest">Latest</option><option value="oldest">Oldest</option><option value="budget">Highest Budget</option></select>
        </div>
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{visibleTrips.map((trip) => <TripCard key={trip.id} trip={trip} />)}</div>
      </div>
    </main>
  );
}
